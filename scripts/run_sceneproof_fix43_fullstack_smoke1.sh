...
manifest="${SCENEPROOF_FIX43_MANIFEST:-/tmp/sceneproof_fix43_fullstack_smoke1.txt}"
...
candidate="${SCENEPROOF_FIX43_GUARDED_VERSION:-v5_sceneproof_fix43_guarded_smoke1_fix53}"
target="${SCENEPROOF_FIX43_CERTIFIED_VERSION:-v5_sceneproof_fix43_certified_smoke1_fix53}"
audit="$root/sceneba_audit/$target"
render_log="logs/${target}_locked_camera_render"
render_dir="$root/sceneproof_fix43_fullstack_smoke1_renders"
render_archive="$HOME/sceneproof_fix43_fullstack_smoke1_renders.tar.gz"
...
echo "===== FIX43 SMOOTH START $(date) ====="
run_branch "$control" 0

echo "===== FIX43 GUARDED START $(date) ====="
run_branch "$candidate" 1

echo "===== FIX43 CERTIFICATE START $(date) ====="
...
for version in "$control" "$candidate" "$target"; do
...
echo "FIX43_FULLSTACK_FINISHED target=$target"
