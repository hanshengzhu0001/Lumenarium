...
env \
  ...
  IMAGINARIUM_S4_TARGET_VERSION=v4_repair_geometry_audit \
  ...
  IMAGINARIUM_SCENEBA_ASSET_CENTER_CANDIDATES=1 \
  IMAGINARIUM_SCENEBA_ASSET_CENTER_SCALES="${SCENEBA_ASSET_CENTER_SCALES:-0.5,1.0,1.5}" \
  IMAGINARIUM_SCENEBA_SUPPORT_SURFACE_CANDIDATES=1 \
  IMAGINARIUM_GPU_FREE_FLOOR_MB="${SCENEBA_REPAIR_GPU_FREE_FLOOR_MB:-16000}" \
  IMAGINARIUM_S4_SCENE_TIMEOUT=3600 \
  IMAGINARIUM_S4_WORKER_LOG_ROOT=logs/v4_repair_geometry_audit_smoke5 \
