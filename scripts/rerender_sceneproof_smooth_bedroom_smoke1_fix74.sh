#!/usr/bin/env bash
set -euo pipefail

cd "$HOME/Lumenarium"

root="${SCENEPROOF_RESULTS_ROOT:-a10_reusable_results/paper30}"
scene="bedroom_01"
source_version="v4_deepsearch"
smooth_version="v5_sceneproof_fix43_smooth_paper30_fix61"
blender="${IMAGINARIUM_BLENDER:-$HOME/Lumenarium/third_party/blender-4.3.2-linux-x64/blender}"
samples="${SCENEPROOF_RENDER_SAMPLES:-256}"
gpu="${SCENEPROOF_RENDER_GPU:-0}"
gpu_floor="${IMAGINARIUM_GPU_FREE_FLOOR_MB:-16000}"
timeout_seconds="${SCENEPROOF_RENDER_TIMEOUT:-1800}"

source_json="$(find "$root/${scene}_${source_version}_result/S3_pose_inference" \
  -maxdepth 1 -type f -name '*_placement_info.json' -print -quit)"
placement="$root/${scene}_${smooth_version}_result/S4_layout_refinement/${scene}_${smooth_version}_placement_info_s4.json"
old_smooth="$root/${scene}_${smooth_version}_result/S4_layout_refinement/${scene}_${smooth_version}_render_simu.png"
rerender="$HOME/fix43_smooth_bedroom_rerender_fix74.png"
camera_audit="$HOME/fix43_smooth_bedroom_rerender_fix74.camera.json"
fix68="$HOME/fix68_bedroom_rerender_fix73.png"
comparison="$HOME/Lumenarium/a10_reusable_results/paper30/sceneba_audit/sceneproof_render_roundtrip_fix74.json"
log="$HOME/Lumenarium/logs/sceneproof_smooth_bedroom_rerender_fix74.log"
tmp_output="/tmp/sceneproof_smooth_rerender_fix74_${scene}_$$"

test -x "$blender" || { echo "Missing Blender: $blender" >&2; exit 2; }
test -s "$source_json" || { echo "Missing S3 source: $source_json" >&2; exit 2; }
test -s "$placement" || { echo "Missing Smooth placement: $placement" >&2; exit 2; }
test -s "$old_smooth" || { echo "Missing old Smooth render: $old_smooth" >&2; exit 2; }
test -s "$fix68" || { echo "Missing Fix73 render: $fix68" >&2; exit 2; }
mkdir -p "$tmp_output" "$(dirname "$comparison")" "$(dirname "$log")"
trap 'rm -rf -- "$tmp_output"' EXIT

while true; do
  free="$(nvidia-smi -i "$gpu" --query-gpu=memory.free \
    --format=csv,noheader,nounits 2>/dev/null | head -1 | tr -d ' ')"
  if test -n "$free" && test "$free" -ge "$gpu_floor"; then
    break
  fi
  echo "WAIT_GPU_FIX74 gpu=$gpu free=${free:-unknown}MiB"
  sleep 30
done

timeout "$timeout_seconds" env \
  CUDA_VISIBLE_DEVICES="$gpu" \
  IMAGINARIUM_S4_RENDER_ONLY_PLACEMENT="$placement" \
  IMAGINARIUM_S4_RENDER_ONLY_OUTPUT="$rerender" \
  IMAGINARIUM_S4_RENDER_ONLY_AUDIT="$camera_audit" \
  IMAGINARIUM_S4_RENDER_ONLY_SAMPLES="$samples" \
  PYTHONUNBUFFERED=1 \
  LD_LIBRARY_PATH="$HOME/.venvs/lumenarium-py311/lib:${LD_LIBRARY_PATH:-}" \
  "$blender" --background \
    --python modules/S4_blender_layout_and_corr.py -- \
    --obj_placement_info_json_path "$source_json" \
    --output_folder "$tmp_output" \
    > "$log" 2>&1 < /dev/null

test -s "$rerender" || { echo "Smooth rerender was not created: $rerender" >&2; exit 1; }
test -s "$camera_audit" || { echo "Camera audit was not created: $camera_audit" >&2; exit 1; }

"$HOME/.venvs/lumenarium-py311/bin/python" \
  sceneproof_render_roundtrip_compare.py \
  --old-smooth "$old_smooth" \
  --rerender-smooth "$rerender" \
  --fix68 "$fix68" \
  --out "$comparison"

echo "FIX74_OLD_SMOOTH=$(readlink -f "$old_smooth")"
echo "FIX74_RERENDERED_SMOOTH=$(readlink -f "$rerender")"
echo "FIX74_FIX68=$(readlink -f "$fix68")"
echo "FIX74_COMPARISON=$(readlink -f "$comparison")"
echo "FIX74_LOG=$(readlink -f "$log")"
