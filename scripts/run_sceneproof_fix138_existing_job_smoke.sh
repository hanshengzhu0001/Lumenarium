...
artifact_dir="$HOME/Lumenarium/api_smoke_fix138/$job_id/$profile"
mkdir -p "$artifact_dir"
bash scripts/run_sceneproof_frozen_single_job_fix115.sh \
  "$job_id" "$input" "$artifact_dir" 0 "$profile"
echo "FIX138_SMOKE_JOB_ID=$job_id"
echo "FIX138_RESULT=$artifact_dir/result.json"
echo "FIX138_EVALUATION=$artifact_dir/evaluation.json"
echo "FIX138_RENDER=$artifact_dir/render.png"
