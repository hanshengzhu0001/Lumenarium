...
def plot_keypoints(
    ...
    if patch_size != 1:
        src_pts = np.array(src_pts) * patch_size  # + patch_size * 0.5      src_pts.shape (86,2) 转换前：array([[ 9.357142 , 15. ],  [ 9.785714 , 14.142858 ],  转换后array([[ 131 , 210 ],  [ 137 , 198 ],
        tar_pts = np.array(tar_pts) * patch_size  # + patch_size * 0.5

    # src_pts = [cv2.KeyPoint(x, y, 1) for x, y in np.float32(src_pts)]
    src_pts = [cv2.KeyPoint(float(x), float(y), 1) for x, y in src_pts]
    tar_pts = [cv2.KeyPoint(float(x), float(y), 1) for x, y in tar_pts]
