...
def gather(features, index_patches):
    ...
    features = rearrange(features, "b c h w -> b (h w) c")  # torch.Size([12, 256, 1024])

    index_patches = index_patches.clone()
    x, y = index_patches[:, :, 0], index_patches[:, :, 1]   # x和y都是torch.Size([12, 256])
    ...
    # reshape to (BxN, C)
    flatten_features = rearrange(flatten_features, "b n c -> (b n) c")  # torch.Size([3072, 1024])
    mask = rearrange(mask, "b n -> (b n)")  # torch.Size([3072])
    return flatten_features[mask] # torch.Size([1297, 1024])  这个1297指的是mask中为True的数量
