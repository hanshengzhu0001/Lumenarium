...
def plot_keypoints_batch(
    ...
    src_img = convert_tensor_to_image(data.src_img, unnormalize=unnormalize)    # torch.Size([12, 3, 224, 224])
    tar_img = convert_tensor_to_image(data.tar_img, unnormalize=unnormalize)

    if type_data == "gt":
        src_pts = data.src_pts.cpu().numpy()    # torch.Size([12, 256, 2])
