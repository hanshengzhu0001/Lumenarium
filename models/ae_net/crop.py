    ...
    def __call__(self, xyxy_boxes, images):
        ...
        for i in range(batch_size):
            ...
            if torch.isinf(scale):
                # 创建一个全黑的224x224图像
                C, _, _ = image.shape
                image = torch.zeros((C, self.target_size, self.target_size), device=device)
                # 由于图像已经是目标尺寸，不需要修改M_resize_pad
            else:
                # 计算缩放后的尺寸
                scaled_height = int(round(image.shape[-2] * scale.item()))
                scaled_width = int(round(image.shape[-1] * scale.item()))
                ...
                # 使用计算后的尺寸进行缩放
                image = F.interpolate(image.unsqueeze(0), size=(scaled_height, scaled_width))[0]
